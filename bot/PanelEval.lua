...
-- bringing the third panel in yourself, while a vertical pair is finished by
-- a panel FALLING into place -- which needs something under it to clear
-- first. One is a trigger, the other is cascade fuel, and a lumped links
-- scores them the same.
--
-- Concretely: a broken garbage row takes its colours from garbageRowColors,
-- which refuses to repeat left to right, so a freshly converted row can never
-- hold a horizontal pair. All of its value is vertical.
local function linksH(input)
  local board = input.board
  local grid, W, H = board.grid, board.width, board.height
  local count = 0
  for r = 1, H do
    for c = 1, W - 1 do
      local v = grid[r][c]
      if v > 0 and grid[r][c + 1] == v then count = count + 1 end
    end
  end
  return count
end

local function linksV(input)
  local board = input.board
  local grid, W, H = board.grid, board.width, board.height
  local count = 0
  for r = 1, H - 1 do
    for c = 1, W do
      local v = grid[r][c]
      if v > 0 and grid[r + 1][c] == v then count = count + 1 end
    end
  end
  return count
end
...
PanelEval.FEATURES = {
  { key = "matchPotential",   group = "board",  sign =  1, norm = 16, fn = matchPotential },
  { key = "chainPotential",   group = "board",  sign =  1, norm = 16, fn = chainPotential },
  { key = "comboPotential",   group = "board",  sign =  1, norm = 36, fn = comboPotential },
  ...
  { key = "colourVariance",   group = "board",  sign = -1, norm = 32, fn = colourVariance },
  ...
  { key = "garbageSent",      group = "earned", sign =  1, norm = 24, fn = garbageSent },
...
function PanelEval.evaluate(raw, weights, opts)
  ...
  for i = 1, #PanelEval.FEATURES do
    ...
    if w and w ~= 0 then
      ...
      if density and f.perPanel and panels > 0 then
        v = v / panels
      elseif f.norm then
        ...
        -- and a weight cannot be read as that feature's share of the
        -- decision. Divisors are the ones in the JavaScript registry this
        -- file is verified against; they must not drift apart.
        v = v / f.norm
      end
      features[f.key] = v
