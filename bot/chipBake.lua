...
function M.author(g, sr, sc, kind, absSwaps, gb)
  local rl, nn = {}, 0
  ...
  for key, class in pairs(analyze.classify(g, absSwaps, { gb = gb })) do
    if not incl[key] then incl[key] = { math.floor(key/100), key % 100, class } end
  end
  -- garbage-based chips (shoguns): the breaking garbage block is part of the shape — emit its cells as "G"
  if gb then for c = gb.lo, gb.hi do incl[gb.row*100+c] = { gb.row, c, "G" } end end
  local meta = analyze.measure(g, absSwaps, gb)
...
local function symOf(cl)   -- *=don't-care · .=must-empty · @=blocker(solid,not-a-solving-color) · G=garbage · digit=color
...
function M.render(chip)
  ...
  if #chip.swaps <= 1 then
    local swc = {}                                   -- swap-less chips (shoguns) get no swap brackets at all
    if chip.swaps[1] then local o = chip.swaps[1]; swc[o[1]*100+o[2]] = true; swc[o[1]*100+o[2]+1] = true end
...
function M.writeAll(chips)
  ...
  local rows, cat, perKind = {}, {}, {}
  for _, c in ipairs(uniq) do
    rows[#rows+1] = string.format("C[#C+1]={ kind=%q, swaps=%s, tmpl=%s, meta=%s }", c.kind, serSwaps(c.swaps), serTmpl(c.tmpl), serMeta(c.meta))
  ...
  -- Emit the chips in BATCHES, each inside its own function. A single `return {…}` literal puts every chip's constants
  -- in one chunk's constant pool, which overflows LuaJIT's hard 65536-constant limit once the cache gets large. Each
  -- batch function is its own prototype with its own pool, so the cache can grow without bound. `return C` is unchanged
  -- for callers — require("bot.chipCache") still just gets the assembled list.
  local BATCH = 500
  local body = { "local C = {}" }
  for i = 1, #rows, BATCH do
    body[#body+1] = "do local function add(C)"
    for j = i, math.min(i + BATCH - 1, #rows) do body[#body+1] = rows[j] end
    body[#body+1] = "end add(C) end"
  end
  body[#body+1] = "return C"
  ...
  f:write("-- Written in batched builder functions (not one literal) to stay under LuaJIT's 65536-constants-per-chunk limit.\n")
