...
-- A cascade end = a primary COMBO_N end (from comboEnds) where ONE cell is delivered by a secondary riser drop:
-- take a primary end, lift one of its cells up onto a vertical M-tall secondary riser (the held cap), and that
-- whole picture (4 primary + riser + cap) is the cascade end -- the moment before the chain fires. Engine-verified:
-- with the riser present, clearing it must drop the cap and complete exactly N primary.
--   luajit bot/getCascadeEnds.lua [N] [M]
...
local M = tonumber(arg[2]) or 3                 -- riser size (a COMBO_M of secondary)
local W, H = 6, 12
...
-- build a cascade-end board from primary cells + riser cells (1-based, r=1 floor). Fill inert support under any
-- floating panel so it is a settled board.
local function buildEnd(prim, riser)
  ...
  for c = 1, W do local top = 0; for r = 1, H do if g[r][c] ~= 0 then top = r end end   -- inert support under floats
    for r = 1, top do if g[r][c] == 0 then g[r][c] = F end end end
  return g
end

-- the riser is a COMBO_M SHAPE (not a size): pull every comboEnds(M) shape (horizontal/vertical for M=3; L/T/plus
-- once M>=5) as candidate risers, normalized to bottom-left (0,0).
local riserShapes = {}
...
for _, e in ipairs(ends) do
  ...
  local maxR, minC, maxC = 0, 1e9, 0
  for _, cell in ipairs(base) do maxR=math.max(maxR,cell[1]); minC=math.min(minC,cell[2]); maxC=math.max(maxC,cell[2]) end
  local function record(g)
    if primaryAlreadyMatched(g) then return end       -- the primary must not be solved before the riser fires
  ...
  -- insert each RISER SHAPE at every placement; lift each primary cell by the riser's cell-count in its column
  -- (so when the riser clears, gravity drops it back into the end). The engine verifies validity.
  for _, R in ipairs(riserShapes) do
    for br = 1, maxR do for bc = minC - 4, maxC do
