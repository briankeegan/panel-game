...
local function genForBase(B0, ar, ac, n, R)
  ...
  local function consider(g, br, bc, moves)
    ...
    if preMatch then return end                                     -- (a) no pre-existing match
    local str = gridToStr(g)
    if anySwapWins(str, g, n) then return end                       -- (b) no SINGLE swap clears the whole combo
  ...
  for br = math.max(1, ar - R), math.min(H, ar + R) do
    for bc = 1, W - 1 do
      ...
      if moves >= 1 and moves <= R then
        -- the displaced gap either COLLAPSES (settle) or is HELD OPEN by a blocker (support); try both
...
require("bot.chipRegistry").register{ name = "getComboSetups", produce = produce }
