...
--   luajit bot/getComboShapes.lua [N]
...
------------------------------------------------------------------ engine truth (same as comboNgen)
...
------------------------------------------------------------------ board construction with support filler
...
-- Every placement of N line panels in a tight window; auto-fill support under any floating panel (so bent shapes
-- are realizable AND the filler supplies colored swap-partners for free); then try every legal swap. The engine
-- is the final judge; the internal settle+match is just a fast pre-filter. Complete by construction.
...
local function gen(start, depth)
  if depth > N then
    ...
    if minLR ~= 1 then return end                       -- FLOOR-ANCHOR: skip elevated duplicates (same chip on a platform)
...
local function render(rec)
  ...
  for rr = rHi, rLo, -1 do
    local toks = {}
    for cc = cLo, cHi do
      -- synthetic support filler (color 2/3) that isn't one of the swapped cells is arbitrary context -> '*'.
      -- Only line cells, the swap pair, and essential empties get the real engine-tested symbol.
