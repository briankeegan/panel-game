...
-- Reject malformed/hostile ids before we'd ever insert them into the playerbase.
-- generate_new_user_id() emits a decimal string from cs_random() (≤10 digits),
-- so digits-only with a loose upper bound is the right gate.
---@param userId string
---@return boolean
function Server:isWellFormedUserId(userId)
  return type(userId) == "string"
    and #userId >= 1 and #userId <= 20
...
function Server:login(connection, userId, name, ipAddress, port, engineVersion, loginMessage)
  ...
  -- Auto-recover from a stale user_id (e.g. after a server data wipe). The
  -- id in the client's user_id.txt was generated by this same code; if it's
  -- well-formed and the name is free, re-register it as-is. The client's
  -- identity stays stable across wipes — no new id round-trip, no UI flap.
  -- Name still gated by nameTaken so an unknown id can't hijack a live name.
  if userId and userId ~= "need a new user id" and self.playerbase
      and not self.playerbase.players[userId]
      and self:isWellFormedUserId(userId)
      and not self.playerbase:nameTaken("", name) then
    if self.playerbase:addPlayer(userId, name) then
      logger.info("Re-registering client-supplied user_id from " .. ipAddress
        .. " for available name '" .. name .. "' (server data wipe recovery).")
    else
      logger.warn("addPlayer failed for client-supplied id from " .. ipAddress
        .. "; falling back to fresh-id allocation.")
...
function Server:closeConnection(connection, reason)
  ...
  if self.playerToRoom[player] or self.spectatorToRoom[player] or self:findPendingJoinerRoom(player) then
    -- Identity guard: only clear gameplayConnection / route the .connection
    -- pointer if THIS socket is the currently-bound one. A stale socket
    -- finishing its close (after the inline "Replacing gameplay socket"
    -- path already swapped player.gameplayConnection to the new one)
    -- would otherwise nil out the freshly-installed connection, stranding
    -- the player on the server while their client still thinks it's
    -- attached. Mirrors the lobby / spectate guards above.
    if player.gameplayConnection == connection then
      ...
      logger.info("Stale gameplay socket " .. connection.index .. " closed for "
        .. player.name .. "; current gameplay socket unaffected.")
    end
    return
  end
